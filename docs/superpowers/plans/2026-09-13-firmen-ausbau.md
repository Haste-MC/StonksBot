...
- **Weitere Abweichungen vom Plan, aus dem Review:** (a) Die Gründungsansicht ist auch auf Discord nach Klasse geblättert (`firma|gruendung|<klasse>`), nicht nur auf Fluxer – neun Branchen passen in keine Knopfreihe. (b) Unbezahlbare Extras werden nicht deaktiviert: die Ansicht kennt das Guthaben nicht (kein `getBalance` je Ansicht, §8/§9), der Kauf antwortet mit `funds`. (c) Zusätzlich zur bedingten Schreibung gibt es eine In-flight-Sperre je Firmen-ID (`inFlight` in `company.js`, §7): Der Ausbau-Kauf wartet zweimal auf die Bank, ein Klick dazwischen las sonst die schon erhöhte Stufe und kaufte die übernächste. (d) Der Ausbau-Knopf trägt die gezeigte Stufe (`firma|ausbauen|<id>`), die Bestätigung `ja-<id>` – kein `0`/`ja` mehr, damit ein alter Knopf nicht eine inzwischen andere Stufe kauft.
