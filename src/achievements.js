 ...
 *
 * @param {boolean} serverweitErlaubt Ob hier überhaupt serverweite Regeln
 *   vergeben werden dürfen. `state()` reicht `false` durch, solange
 *   `backfillWorld` für diese Welt noch nicht FERTIG ist: Ein Blick auf ein
 *   Profil ist keine Leistung – ohne diese Sperre schnappt sich sonst wieder
 *   der erste Betrachter nach dem Deploy ein serverweites Abzeichen, bevor
 *   der Welt-Nachtrag die Kandidaten überhaupt verglichen hat. `onActivity`
 *   und `fire` lassen den Parameter bewusst weg (Default `true`): Dort hängt
 *   der Erfolg an einer echten Tat, nicht an einem Blick, und darf ganz normal
 *   gewinnen – auch bevor der Welt-Nachtrag gelaufen ist. Wer serverweit
 *   gewinnt, soll das also entweder durch eine Tat oder durch den Vergleich
 *   im Welt-Nachtrag selbst tun.
 * @returns {Array} die frisch vergebenen Regeln (zum Melden)
 */
function check(guildId, userId, hook, ctx, now = Date.now(), serverweitErlaubt = true) {
  ...
  for (const rule of RULES) {
    ...
    if (rule.scope === 'server' && !serverweitErlaubt) continue;
